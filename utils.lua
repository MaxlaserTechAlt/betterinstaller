...
functions.GetTarget = function(args: table?): table
    ...
    for i,v in players:GetPlayers() do 
        if shared.GuiLibrary and not RenderLibrary.whitelist:get(2, v) then 
            continue 
        end;
...
functions.isEnabled = function(button: string, category: string?): boolean
	local success, enabled = pcall(function()
		return shared.GuiLibrary.ObjectsThatCanBeSaved[button..(category or 'OptionsButton')].Api.Enabled 
	end)
	return success and enabled
end;

functions.GetAllTargets = function(distance: number?, mobs: boolean?, raycast: boolean?, sort)
	local targets = {}
	local entities = {
		Monster = collection:GetTagged('Monster'),
		DiamondGuardian = collection:GetTagged('DiamondGuardian'),
...
functions.warningNotification = function(title: string, text: string, delay: number): Frame
	local suc, res = pcall(function()
		local color = shared.GuiLibrary.ObjectsThatCanBeSaved['Gui ColorSliderColor'].Api
		local frame = shared.GuiLibrary.CreateNotification(title, text, delay, 'assets/WarningNotification.png')
...
functions.InfoNotification = function(title: string, text: string, delay: number): Frame
	local success, frame = pcall(function()
		return shared.GuiLibrary.CreateNotification(title, text, delay)
	end)
...
functions.errorNotification = function(title: string, text: string, delay: number): Frame
	local success, frame = pcall(function()
		local notification = shared.GuiLibrary.CreateNotification(title, text, delay or 6.5, 'assets/WarningNotification.png')
...
functions.hookfunction = hookfunction or hookfunc or hook_function or function(old, new)
    pcall(function()
        getfenv(old)[debug.info(old, 'n')] = new;
    end);
end;
